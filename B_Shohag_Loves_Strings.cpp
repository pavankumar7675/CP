...
void solve() {
    ...
    for(int d = 0; d < b; d++){
        int e = 0;
        c += a[d];
        while(d < b-1 && a[d] == a[d+1]){
            d++;
            e = 1;
        }
        if(e){
            cout << c[c.length()-1] << c[c.length()-1] << endl;
            return;
            c += c[c.length()-1];
        }
    }
    if(c.length() < 3){
        ...
        for(int f = 0; f < c.length()-2; f++){
            if(fu(a[f], a[f+1], a[f+2])){
                cout << c.substr(f, 3) << endl;
                return;
            }
        }
    }
